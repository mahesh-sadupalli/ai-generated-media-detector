...
from src.artifact_detectors.mode_collapse_detector import ModeCollapseDetector
...
def test_mode_collapse_detector_on_dataset():
    ...
    adversarial_dir = DATA_DIR / "generated" / "high_adversarial"
    if adversarial_dir.exists():
        adversarial_scores = []
        adversarial_files = sorted(f for f in os.listdir(adversarial_dir) if f.endswith('.jpg'))[:5]

        for filename in adversarial_files:
            img_path = adversarial_dir / filename
