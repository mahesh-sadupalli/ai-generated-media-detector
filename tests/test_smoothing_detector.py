...
from src.artifact_detectors.smoothing_detector import SmoothingArtifactDetector
...
def test_detector_on_dataset():
    ...
    pixel_dir = DATA_DIR / "generated" / "high_pixel"
    if pixel_dir.exists():
        pixel_scores = []
        pixel_files = sorted(f for f in os.listdir(pixel_dir) if f.endswith('.jpg'))[:5]

        for filename in pixel_files:
            img_path = pixel_dir / filename
